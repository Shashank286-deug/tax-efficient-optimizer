...
with tab3:
    st.header("🧠 Quant Concepts Explained")
    st.markdown("This section details the mathematical models used in this dashboard.")
    
    st.subheader("1. Monte Carlo Simulation")
    st.markdown("""
    Instead of relying on a single prediction, we use **Monte Carlo Simulation** to generate 5,000 random portfolio combinations. 
    By simulating thousands of different weightings, we can mathematically locate the "Efficient Frontier"—the set of portfolios that offer the highest expected return for a defined level of risk.
    """)
    
    st.subheader("2. Value at Risk (VaR)")
    st.markdown("""
    **Definition:** VaR estimates the maximum potential loss over a specific time frame at a given confidence level (95%).
